...
@mcp.tool()
async def get_jockey_profile(jockey_id: str) -> str:
    """騎手のプロフィール情報を取得する関数
    https://db.netkeiba.com/jockey/{jockey_id}/ から取得する

    Input:
        jockey_id: str - 取得したい騎手のID
    ...
    """
    html = await get_jockey_profile_html(jockey_id)
    jockey_profile = parse_jockey(html)

    return jockey_profile.model_dump_json()
